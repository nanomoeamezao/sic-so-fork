...
class SynchronisedSkillScript : BaseEveryFrameCombatPlugin() {
    ...
    var intervalHullArmor = IntervalUtil(0.5f, 0.5f)
    var intervalAmmo = IntervalUtil(10f, 10f)
    ...
    override fun advance(amount: Float, events: MutableList<InputEventAPI>?) {
        ...
        if (playership != null && originalShip != null) {
            val distanceSquared = getDistanceSquared(playership, originalShip)
            val missingAmmo = playership.allWeapons.any { it.usesAmmo() && it.ammo < it.maxAmmo }

            if (distanceSquared <= REGENERATION_RANGE * REGENERATION_RANGE) {
                intervalHullArmor.advance(amount)
                if (missingAmmo) intervalAmmo.advance(amount)

                if (intervalHullArmor.intervalElapsed()) {
                    regenerateHullAndArmor()
                }
                if (missingAmmo && intervalAmmo.intervalElapsed()) {
                    replenishAmmo()
                }
                if (!missingAmmo) intervalAmmo.elapsed = 0f
            }
            else {
                intervalHullArmor.elapsed = 0f
                intervalAmmo.elapsed = 0f
            }
        }
    ...
    private var REGENERATION_RANGE = 500f
    private var HULL_REGEN_AMOUNT = .033f
    private var ARMOR_REGEN_AMOUNT = .033f
    var AMMO_REGEN_AMOUNT = 1

    private fun regenerateHullAndArmor() {
        val playerShip = Global.getCombatEngine().playerShip

        if (playerShip != null && playerShip.isFighter && originalShip != null) {
            val distanceSquared = getDistanceSquared(playerShip, originalShip)
            if (distanceSquared <= REGENERATION_RANGE * REGENERATION_RANGE) {
                // Regenerate hull
                playerShip.hitpoints = Math.min(playerShip.hitpoints + (playerShip.maxHitpoints * HULL_REGEN_AMOUNT), playerShip.maxHitpoints)

                // Regenerate armor in all cells
                val armorGrid = playerShip.armorGrid
                for (x in 0 until armorGrid.grid.size) {
                    for (y in 0 until armorGrid.grid[x].size) {
                        val regenAmount = armorGrid.maxArmorInCell * ARMOR_REGEN_AMOUNT
                        armorGrid.setArmorValue(
                            x,
                            y,
                            Math.min(armorGrid.getArmorValue(x, y) + regenAmount, armorGrid.maxArmorInCell)
                        )
                    }
                }
            }
    ...
    private fun replenishAmmo() {
        val playerShip = Global.getCombatEngine().playerShip

        if (playerShip != null && playerShip.isFighter && originalShip != null) {
            val distanceSquared = getDistanceSquared(playerShip, originalShip)
            if (distanceSquared <= REGENERATION_RANGE * REGENERATION_RANGE) {
                // Regenerate ammo for all weapons
                for (weapon in playerShip.allWeapons) {
                    if (weapon.usesAmmo()) {
