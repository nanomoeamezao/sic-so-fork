...
class SCSkillMenuPanel(var parent: UIPanelAPI, var data: SCData, var title: Boolean,/* var seedTextElement: LabelAPI, var seedElement: UIComponentAPI, var copyButton: UIComponentAPI*/) {
    ...
    companion object {
        var crCost : Float = a9kSCSettings.crPenalty
    }
    ...
    fun addAptitudeRowParent(targetedElelement: TooltipMakerAPI, officer: SCOfficer?, slotId: Int) {
        var subpanel = Global.getSettings().createCustom(width, elementSize, null)
    ...
    fun recreateAptitudeRow(subpanelParent: CustomPanelAPI, officer: SCOfficer?, slotId: Int) {
        ...
        var subelement = subpanel.createUIElement(width, elementSize, false)
        subpanel.addUIElement(subelement)
        ...
        var officerPickerElement = SCOfficerPickerElement(officer?.person, color, subelement, elementSize, elementSize)
        ...
        var officerUnderline = SkillUnderlineElement(color, 2f, subelement, elementSize)
        ...
        aptitudePara.position.inTL(paraElement.width / 2 - aptitudePara.computeTextWidth(aptitudePara.text) / 2 - 3, paraElement.height  )
...
*/
        ...
        var originSkillElement = SkillWidgetElement(originSkill!!.id, aptitudePlugin.id, true, false, true, originSkill!!.iconPath, "leadership1", aptitudePlugin.getColor(), subelement, elementSize, elementSize)
        ...
        for (section in sections) {
            ...
            for (skill in skills) {
                ...
                var skillElement = SkillWidgetElement(skill, aptitudePlugin.id, activated, !preacquired, preacquired, skillPlugin!!.getIconPath(), section.soundId, aptitudePlugin.getColor(), subelement, elementSize, elementSize)
                skillElements.add(skillElement)
                section.activeSkillsInUI.add(skillElement)
                usedWidth += elementSize
